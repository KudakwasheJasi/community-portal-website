...
export class EventsService {
    ...
    return [
      {
        ...
        description: 'Join us for an exciting discussion about the latest in web development technologies.',
      ...
      } as unknown as Event,
      {
        ...
        endDate: new Date(futureDate.getTime() + 24 * 60 * 60 * 1000 + 3 * 60 * 60 * 1000), // 3 hours later
        maxAttendees: 25,
        status: EventStatus.PUBLISHED,
      ...
      } as unknown as Event,
      {
        ...
        description: 'Connect with fellow community members and build professional relationships.',
        location: 'Downtown Conference Center',
        startDate: new Date(futureDate.getTime() + 2 * 24 * 60 * 60 * 1000), // Two days from now
        endDate: new Date(futureDate.getTime() + 2 * 24 * 60 * 60 * 1000 + 4 * 60 * 60 * 1000), // 4 hours later
        maxAttendees: 100,
        status: EventStatus.PUBLISHED,
  ...
  async update(id: string, updateEventDto: UpdateEventDto, userId: string): Promise<Event> {
    ...
    if (updateEventDto.maxAttendees !== undefined) updateData.maxAttendees = updateEventDto.maxAttendees;
